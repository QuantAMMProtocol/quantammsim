"""Train latent SDE with drifting loss + drift features, compare drift/vol."""
...
# --- Train with drifting loss + drift features ---
print(f"\n=== Training LATENT SDE with DRIFTING LOSS + DRIFT FEATURES ===")
print(f"    (mc_samples=64, 500 steps, auto drift_weight)")
...
latent_sde, history = fit_latent_sde_drifting(
    ...
    lr=1e-3,
    weight_decay=1e-4,
    drift_weight=None,  # auto-compute from data
    patience=300,
    verbose=True,
)
...
# Drift network diagnostic
print(f"\n=== Drift network output diagnostic ===")
print(f"  Bias (constant):  {[f'{float(latent_sde.drift.bias[j]):.6f}' for j in range(n_assets)]}")
print(f"  Max dev (bound):  {[f'{float(latent_sde.drift.max_dev[j]):.6f}' for j in range(n_assets)]}")
print(f"  Real drift:       {[f'{float(real_drift[j]):.6f}' for j in range(n_assets)]}")
test_points = daily_log[::100]
for idx in range(min(5, test_points.shape[0])):
    y = test_points[idx]
    z = latent_sde.encoder(y)
    f_total = latent_sde.drift(z)
    f_mlp_only = latent_sde.drift.mlp(z)
    observed_drift = latent_sde.readout.weight @ f_total
    mlp_contrib = latent_sde.readout.weight @ f_mlp_only

    print(f"\n  Day ~{idx*100}:")
    print(f"    Observed drift (W @ f(Z)):  {[f'{float(observed_drift[j]):.6f}' for j in range(n_assets)]}")
    print(f"    MLP contribution (W @ mlp): {[f'{float(mlp_contrib[j]):.6f}' for j in range(n_assets)]}")
...
# Readout weight analysis
print(f"\n=== Readout weight (deviation from identity) ===")
W = latent_sde.readout.weight
identity_part = W[:, :n_assets]
hidden_part = W[:, n_assets:]
print(f"  Asset block (should be near I):\n{np.array(identity_part)}")
print(f"  Hidden block (learned coupling):\n{np.array(hidden_part)}")
print(f"  Hidden block norm: {float(jnp.linalg.norm(hidden_part)):.6f}")
