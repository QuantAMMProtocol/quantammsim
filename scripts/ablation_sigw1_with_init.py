"""Ablation C: drift init + Sig-W1 loss (no drifting loss, no drift features).

Tests whether correct initialisation + standard Sig-W1 loss is sufficient,
or whether the drifting loss is needed to preserve drift during training.
"""
...
# --- Ablation C: drift init + Sig-W1 loss ---
print(f"\n=== ABLATION C: Drift init + Sig-W1 loss (no drifting) ===")
print(f"    (mc_samples=64, 500 steps)")
key = jax.random.PRNGKey(42)
latent_sde, history = fit_latent_sde_sigw1(
    minute_prices_jnp,
    n_assets=n_assets,
    key=key,
    n_hidden=4,
