...
def resample_to_minute_grid_reindex(df, timestamp_col="datetime"):
    ...
    minute_range = pd.date_range(
        start=df_indexed.index.min(), end=df_indexed.index.max(), freq="1T"
    )
...
def load_scraped_pool_data(data_dir="./sonic_macro"):
    ...
    print(
        f"Original data range: {combined_df['datetime'].min()} to {combined_df['datetime'].max()}"
    )
    ...
    coarse_weights["weights"] = coarse_weights["weights"] / np.sum(
        coarse_weights["weights"], axis=1, keepdims=True
    )
    # Create fee arrays
...
def plot_weights(
    ...
    pl = (
        ...
        .label(
            y="$\\mathrm{Weight}$",
            x="$\\mathrm{Time}$" if not is_monthly else "$\\mathrm{Date}$",
        )
    )
...
def plot_values(
    ...
    suffix="",
    plot_start_end=None,
    plot_white_line=False,
    white_line_date=None,
    plot_dir=None,
    initial_hodl_weights="same",
):
    ...
    if plot_white_line:
        ...
        plt.text(
            pd.Timestamp(white_line_date) - pd.Timedelta(days=15),
            plt.ylim()[1] * 0.95,
            "$\\mathrm{Train}$",
    ...
    plt.savefig(
        plot_path
        / (f"pool_values_comparison_{len(results)}_{'-'.join(tokens)}_{suffix}.png"),
        dpi=700,
        bbox_inches="tight",
    )
    return_over_hodl = (
        df[df["Strategy"] != name_to_latex_name("HODL")]["Value"].iloc[-1]
        * 1e6
        / hodl_values[-1]
        - 1.0
    )
...
def plot_reserves(
    result, fingerprint, run_name, actual_reserves_np=None, actual_unix_values=None
):
    ...
    if actual_reserves_np is not None and actual_unix_values is not None:
        ...
        for i, col in enumerate(reserve_cols):
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(PLOT_DIR, f"reserves_vs_real_{run_name}_{col}.png"),
                dpi=700,
                bbox_inches="tight",
            )
            plt.close()
...
def create_config_from_scraped_data(data_dir="./sonic_macro"):
    ...
    end_date = datetime.fromtimestamp(last_timestamp / 1000, tz=timezone.utc).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
...
def create_config_with_actual_reserves(data_dir="./sonic_macro"):
    ...
    coarse_weights = {
        ...
        "unix_values": jnp.array(
            scraped_data["full_data"]["timestamp"].iloc[sampled_indices].values
        ),
    }
...
def example_usage():
    ...
    print(
        f"Date range: {scraped_data['full_data']['timestamp'].min()} to {scraped_data['full_data']['timestamp'].max()}"
    )
...
if __name__ == "__main__":
    ...
    list_of_date_variations = list_of_date_variations + generate_daily_variations(
        start_date, end_date
    )
    ...
    for date_variation in list_of_date_variations:
        ...
        config["params"]["initial_reserves"] = filter_reserves_by_given_timestamp(
            ...
            datetime_to_unixtimestamp(
                date_variation["startDateString"], str_format="%Y-%m-%d %H:%M:%S"
            )
            * 1000,
        )
        ...
        datetime_array = pd.date_range(
            start=start_datetime, end=end_datetime, freq="1min"
        )[:-1]
