...
class MomentumDynamicFeeHook(BaseDynamicFeeHook):
    """Dynamic fee hook that adjusts pool fees based on price momentum.

    Extends :class:`BaseDynamicFeeHook` to compute fees from short-term
    price gradients.  The fee for each asset is
    ``sigmoid(scale * gradient + bias)`` where ``scale`` and ``bias`` are
    learnable parameters, giving the optimiser control over fee sensitivity
