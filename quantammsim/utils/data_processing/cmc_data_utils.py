...
def import_cmc_data(file_path):
    ...
    required_columns = ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'marketCap']
    missing_columns = [col for col in required_columns if col not in df.columns]
    if missing_columns:
        raise ValueError(f"Missing required columns: {missing_columns}")

    # # Convert string columns to numeric, removing quotes if present
    # numeric_columns = ['open', 'high', 'low', 'close', 'volume', 'marketCap']
    # for col in numeric_columns:
    #     df[col] = pd.to_numeric(df[col].str.replace('"', ''), errors='coerce')
    return df

def process_cmc_timestamps(df):
...
def forward_fill_cmc_data(df, token):
...
def load_and_process_cmc_data(file_path, token="PAXG", root_path=None):
    ...
    if root_path:
        base_path = Path(root_path) / f"{token}_7D_graph_coinmarketcap.csv"
        new_path = (
            Path(root_path) / "new" / f"{token}_7D_graph_coinmarketcap.csv"
        )
        ...
        if base_path.exists() and new_path.exists():
            ...
            base_df = import_cmc_data(str(base_path))
            new_df = import_cmc_data(str(new_path))
            ...
            raw_df = import_cmc_data(file_path)
    else:
        # No root_path directory specified, proceed with normal import
        raw_df = import_cmc_data(file_path)
    # Process timestamps
    processed_df = process_cmc_timestamps(raw_df)
    ...
    # Forward fill data
    filled_df = forward_fill_cmc_data(processed_df, token)
...
def fill_missing_rows_with_cmc_historical_data(concatenated_df, root_path, token="PAXG"):
    ...
    file_path = Path(root_path) / f"{token}_7D_graph_coinmarketcap.csv"
    ...
    historical_data = load_and_process_cmc_data(file_path, token, root_path)
    ...
    #     print(f"Error filling missing rows with {token} historical data: {str(e)}")
    #     return concatenated_df, None
