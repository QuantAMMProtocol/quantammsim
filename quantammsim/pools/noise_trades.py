...
def calculate_reserves_after_noise_trade(
    ...
    """Calculate pool reserves after accounting for noise trader fee income following a simple model.
    ...
    noise_trader_ratio : float
        Ratio of noise trader to arbitrageur volume (e.g. 1.5 means noise traders contribute 60% of volume)
    gamma : float
    ...
    where the weights change with time, as even if there is a CFMM pool with the same constituents as the
    TFMM pool, one cannot expect that the noise trading behaviour will be the same as for a CFMM pool (i.e.
    with static weights), say. This approach also works for pools with more than two assets, and does not
    ...
    .. [2] "Arbitrageurs' profits, LVR, and sandwich attacks: batch trading as an AMM design response"
           section 5.3
    ...
    # following the results in Appendix B of the paper "When does the tail wag the dog? Curvature and market making"
    ...
    # In "Arbitrageurs' profits, LVR, and sandwich attacks: batch trading as an AMM design response."
