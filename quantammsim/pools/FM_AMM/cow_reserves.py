...
if __name__ == "__main__":
    ...
    for fees in [0.01, 0.1]:
        for noise_trader_ratio in [0.0, 0.5, 1.0]:
            for arb_quality in [0.0, 0.5, 1.0]:
                if fees == 0.0:
                ...
                result = _jax_calc_cowamm_reserves_with_dynamic_fees_and_trades_scan_function(
                    [prev_prices, initial_reserves],
                    [prices, 1.0 - fees, arb_thresh, arb_fees, []],
                    weights=jnp.array([0.8, 0.2]),
                    arb_quality=arb_quality,
                    noise_trader_ratio=noise_trader_ratio,
                    do_trades=False,
                )[-1]
                print(f"{fees},{noise_trader_ratio},{arb_quality}: {result}")
