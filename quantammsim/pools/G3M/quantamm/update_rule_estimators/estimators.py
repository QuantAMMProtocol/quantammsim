...
from quantammsim.pools.G3M.quantamm.update_rule_estimators.estimator_primitives import (
    ...
    squareplus,  # noqa: F401  — re-exported to pool modules
)
...
def calc_gradients(
    ...
    if use_alt_lamb:
        if update_rule_parameter_dict.get("logit_delta_lamb") is not None:
            ...
            if cap_lamb:
                capped_alt_lamb = jnp.clip(alt_lamb, min=0.0, max=max_lamb)
        ...
        alt_memory_days = jnp.clip(alt_memory_days, min=0.0, max=max_memory_days)
    else:
...
def calc_ewma_pair(
    ...
    if cap_lamb:
        max_lamb = jax_memory_days_to_lamb(max_memory_days, chunk_period)
        lamb_1 = jnp.clip(lamb_1, min=0.0, max=max_lamb)
        lamb_2 = jnp.clip(lamb_2, min=0.0, max=max_lamb)
