...
def calculate_jensens_alpha(portfolio_returns, rf_values, benchmark_returns):
    ...
    beta, alpha, r_value, p_value, std_err = linregress(excess_benchmark_returns, excess_portfolio_returns)
...
def calculate_sharpe_ratio(portfolio_returns, rf_values):
    ...
    trading_days_per_year = 252  # Typically, there are 252 trading days in a year
    annualized_sharpe_ratio = sharpe_ratio * np.sqrt(trading_days_per_year)

    return { 'sharpe_ratio': sharpe_ratio, 'annualized_sharpe_ratio': annualized_sharpe_ratio }
...
def calculate_tracking_error_and_information_ratio(portfolio_returns, benchmark_returns):
    ...
    tracking_error = np.std(excess_returns, ddof=1)  # ddof=1 provides sample standard deviation
...
def calculate_drawdown_statistics(daily_returns, rf_values):
    ...
    cDaR = calculate_cdar(daily_returns)
...
def calculate_omega_ratio(portfolio_returns, rf_values, threshold=0):
    ...
    return { 'Omega Ratio': omega_ratio, 'Probability Positive': probability_positive, 'Probability Negative': probability_negative } 
...
def calculate_calmar_ratio(portfolio_returns, rf_values):
    ...
    # Calculate the excess returns by subtracting risk-free rate
    excess_returns = portfolio_returns - rf_values

    # Calculate the cumulative returns
    cumulative_returns = np.cumprod(1 + excess_returns)

    # Calculate the annualized return
    total_return = cumulative_returns[-1]
    num_days = len(excess_returns)
    annualized_return = (total_return ** (trading_days_per_year / num_days)) - 1
...
def calculate_capture_ratios(portfolio_returns, benchmark_returns):
    ...
    # Calculate portfolio returns during positive and negative benchmark return periods
