...
class TestNoiseTraderRatio:
    ...
    def test_noise_trader_ratio_increases_fee_revenue(self):
        """Noise traders add fee-bearing flow, so LP fee revenue should be higher."""
        res_no_noise, rev_no_noise = self._run_with_noise(0.0)
        res_noise, rev_noise = self._run_with_noise(0.1)

        assert float(rev_noise.sum()) > float(rev_no_noise.sum()), (
            f"Noise traders should increase LP fee revenue: {rev_noise.sum()} <= {rev_no_noise.sum()}"
        )
    ...
    def test_noise_trade_does_not_affect_virtual_balances(self):
        ...
        inputs = [
            ...
            0.0,  # arb_thresh
            0.0,  # arb_fees
            jnp.array([0.0, 0.0, 0.0, jnp.nan]),  # price_ratio_update (no-op)
            jnp.float64(1.0),  # lp_supply
            jnp.float64(1.0),  # hypersurge_peg_ratio
        ]
...
class TestLpSupply:
    ...
    def test_lp_supply_through_pool_class(self):
        ...
        di_with_lp = DynamicInputArrays(
            ...
            hypersurge_peg=jnp.ones(n_steps),
        )
