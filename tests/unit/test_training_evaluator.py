...
# Note: Early pruning tests removed - pruning is now handled by the consumer
# (HyperparamTuner) via Optuna's pruning mechanism, not by TrainingEvaluator.
# See TestHyperparamTuner for pruning tests.
...
class TestEdgeCases:
    ...
    def test_inf_wfe_flows_through_aggregation(self):
        """Infinite WFE values should flow through (not be filtered)."""
        evaluator = TrainingEvaluator.random_baseline(verbose=False)
        ...
        # inf flows through - consumer (hyperparam_tuner) handles bad values
        assert np.isinf(result.mean_wfe)
